...
SamplerState gShadowMapSamplers[] : register(s0, SPACE(SCENE));

Texture2D gEnvMap : register(t5, SPACE(PASS));
SamplerState gEnvMapSampler : register(s5, SPACE(PASS));

TextureCube gPrefileteredEnvMap : register(t6, SPACE(PASS));
SamplerState gPrefileteredEnvMapSampler : register(s6, SPACE(PASS));

Texture2D gLUT : register(t7, SPACE(PASS));
SamplerState gLUTSampler : register(s7, SPACE(PASS));
...
float ShadowFeeler(float3 posWorld, LightData ld)
{
    ...
    if(uv.x < 0.0f || uv.x > 1.0f || uv.y < 0.0f || uv.y > 1.0f)
        return 0.0f;
    float depth = gShadowMaps[ld.mTextureIndex].Sample(gShadowMapSamplers[ld.mTextureIndex], TransformUV(uv)).r;
...
float3 SpecularIBL(float3 SpecularColor, float roughness, float3 N, float3 V)
{
    ...
    for (uint i = 0; i < NumSamples; i++)
    {
        ...
        if (NoL > 0)
        {
            float3 ProbeColor = gEnvMap.SampleLevel(gEnvMapSampler, TransformUV(DirToUV(L)), 0).rgb;
...
float3 ApproximateSpecularIBL(float3 SpecularColor, float roughness, float3 N, float3 V)
{
    ...
    float4 PrefilteredColor = gPrefileteredEnvMap.SampleLevel(gPrefileteredEnvMapSampler, L, roughness * uScene.mPrefilteredEnvMapMipLevelCount);
    float4 EnvBRDF = gLUT.SampleLevel(gLUTSampler, TransformUV(float2(NoV, roughness)), 0);
...
float3 Lighting(SurfaceDataIn sdi)
{
    ...
    for (uint i = 0; i < lightCount && i < LIGHT_COUNT_PER_SCENE_MAX; i++)
    {
