...
void main(uint3 gGroupID : SV_GroupID, uint gGroupIndex : SV_GroupIndex)
{
	...
	if (FaceCellExist(faceCellIndex))
	{
		...
		// put this in here so that we can slip in a write again barrier to make sure all cell's velocity is updated
		...
		{
			uint cellIndex = FlattenCellIndexClamp(faceCellIndexXYZ);
