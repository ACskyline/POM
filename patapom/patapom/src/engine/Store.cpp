...
int Store::EstimateTotalSamplerCount(int frameCount)
{
	...
	for (auto scene : mScenes)
		totalSamplerCount += scene->GetSamplerCount();
	return totalSamplerCount * frameCount;
}
...
void Store::RefreshStaticDescriptors(int frameIndex)
{
	for (auto pass : mPasses)
		pass->RefreshStaticDescriptors(frameIndex);
}
