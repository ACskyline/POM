...
CommandLineArg PARAM_loadRawShaders("-loadRawShaders");

Shader::Shader(const ShaderType& type, const string& fileName) :
	mType(type),
	mFileName(fileName)
{
}
...
void Shader::CreateShaderFromFile(const string& fileName)
{
	HRESULT hr;
	...
	if (PARAM_loadRawShaders.Get()) // compile on the fly
	{
		// compile shader
		...
		hr = D3DCompileFromFile(StringToWstring(ShaderSrcPath + fileName + ".hlsl").c_str(),
			nullptr,
	...
	else // load compiled binary
	{
		hr = D3DReadFileToBlob(StringToWstring(ShaderBuildPath + fileName + ".cso").c_str(), &mShader);
		if (FAILED(hr))
		{
			CheckError(hr, nullptr);
			fatalf("shader compiling failed");
		}
	}
