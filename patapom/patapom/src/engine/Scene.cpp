...
void Scene::AddSampler(Sampler* sampler)
{
...
DescriptorHeap::Handle Scene::GetCbvSrvUavDescriptorHeapTableHandle(int frameIndex)
{
	return mCbvSrvUavDescriptorHeapTableHandles[frameIndex];
}

DescriptorHeap::Handle Scene::GetSamplerDescriptorHeapTableHandle(int frameIndex)
{
	...
	DescriptorHeap& samplerDescriptorHeap)
{
	...
	{
		// scene texture table
		mCbvSrvUavDescriptorHeapTableHandles[i] = cbvSrvUavDescriptorHeap.GetCurrentFreeHandle();
		for (auto texture : mTextures)
			cbvSrvUavDescriptorHeap.AllocateSrv(texture->GetTextureBuffer(), texture->GetSrvDesc());
		
		// scene sampler table
		mSamplerDescriptorHeapTableHandles[i] = samplerDescriptorHeap.GetCurrentFreeHandle();
		for (auto sampler : mSamplers)
			samplerDescriptorHeap.AllocateSampler(sampler->GetDesc());
	}
}
...
void Scene::CreateUniformBuffer(int frameCount)
{
	...
	for (int i = 0; i < frameCount; i++)
	{
		...
		mUniformBuffers[i]->SetName(StringToWstring(mDebugName + ": scene uniform buffer " + to_string(i)).c_str());
	}
}
...
void Scene::UpdateUniform()
{
	...
	for (int i = 0; i < mLights.size() && i < LIGHT_PER_SCENE_MAX; i++)
